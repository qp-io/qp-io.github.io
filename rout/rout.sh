...
# ============================================================
#  Конфиг (файл хранит наши relay)
#  Формат строки: id|in_port|proto|target_host|target_port|target_ip|added_at
# ============================================================
...
# ============================================================
#  Cron — авто-обновление IP для доменных relay
# ============================================================
...
while IFS='|' read -r id in_port proto target_host target_port old_ip added_at; do
    # Пропускаем если target — это уже IP
    ...
    [[ -z "$new_ip" ]] && { log "[$id] Не удалось разрезолвить $target_host"; continue; }
    ...
    if [[ "$new_ip" != "$old_ip" ]]; then
        ...
        # Обновляем конфиг
        sed -i "s|^$id|$in_port|$proto|$target_host|$target_port|$old_ip|.*|$id|$in_port|$proto|$target_host|$target_port|$new_ip|$(date '+%Y-%m-%d %H:%M:%S')|" "$CONF_FILE"
        # sed с | разделителем может не сработать — используем Python
        python3 -c "
import re, sys
...
for l in lines:
    ...
    if parts[0] == '$id':
        parts[5] = '$new_ip'
        parts[6] = '$(date \"+%Y-%m-%d %H:%M:%S\")'
        out.append('|'.join(parts) + '\n')
    else:
        out.append(l)
open('$CONF_FILE', 'w').writelines(out)
"
        # Сохраняем правила
...
show_status() {
    ...
    if [[ -z "$relays" ]]; then
        ...
        while IFS='|' read -r id in_port proto target_host target_port target_ip added_at; do
            # Проверяем активно ли правило в iptables
            local active="${RED}✗${NC}"
            if iptables -t nat -L PREROUTING -n 2>/dev/null | grep -q "$target_ip:$target_port"; then
                active="${GREEN}✓${NC}"
            fi
...
do_install() {
    ...
    # Устанавливаем cron если target — домен
...
do_remove() {
    ...
    if [[ "$choice" == "all" ]]; then
        ...
        # Если доменных relay больше нет — убираем cron
        if ! grep -v "^[0-9]\{1,3\}\." "$CONF_FILE" 2>/dev/null | grep -q "|"; then
            local remaining_domains
            remaining_domains=$(while IFS='|' read -r _ _ _ th _ _ _; do
                is_ip "$th" || echo "$th"
            done < <(conf_list))
            [[ -z "$remaining_domains" ]] && remove_cron
        fi
    fi
...
self_uninstall() {
    ...
    # Сам бинарник — в последнюю очередь
    if [[ -f "$SELF_PATH" ]]; then
        rm -f "$SELF_PATH"
...
show_menu() {
    ...
    # Показываем пункт установки только если ещё не установлен
    local installed=""
    [[ -f "$SELF_PATH" && "$(realpath "$0")" == "$SELF_PATH" ]] && installed=1
