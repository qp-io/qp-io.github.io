...
function show_help {
  ...
  -t, --transport <протокол>   Транспорт VLESS: tcp/http/xhttp/grpc/ws (по умолчанию: ${defaults[transport]})"
...
function build_config {
  ...
  if [[ ${config[protocol]} == 'hysteria2' && ${config[security]} == 'reality' ]]; then
    echo 'Протокол "hysteria2" несовместим с "reality". Используйте letsencrypt или selfsigned.'
    exit 1
  fi
  if [[ ${config[protocol]} == 'hysteria2' && ${config[security]} == 'letsencrypt' && ! ${config[server]} =~ ${regex[domain]} ]]; then
    echo 'Для hysteria2 с letsencrypt нужен домен в --server.'
    exit 1
  fi
...
function generate_engine_config {
  ...
  # ─────────────────────────────────────────────────────────────
  # Hysteria 2 — QUIC/UDP
  # Структура согласно PR #5679 (автор LjhAUMEM, протестировано):
  #   protocol="hysteria", network="hysteria", alpn=["h3"]
  # ─────────────────────────────────────────────────────────────
  if [[ ${config[protocol]} == 'hysteria2' ]]; then
    ...
    if [[ ${config[security]} == 'notls' ]]; then
      hy2_tls_block='"security": "none"'
    else
      # selfsigned или letsencrypt — TLS с alpn h3
      hy2_tls_block='"security": "tls",
      "tlsSettings": {
        "alpn": ["h3"],
        "certificates": [{
          "oneTimeLoading": true,
...
function generate_config {
  ...
  # Для hysteria2: сертификат нужен напрямую в engine (без haproxy, UDP/QUIC)
  if [[ ${config[protocol]} == 'hysteria2' && ${config[security]} != "reality" && ${config[security]} != "notls" ]]; then
    mkdir -p "${config_path}/certificate"
...
function print_client_configuration {
  ...
  if [[ ${config[protocol]} == 'hysteria2' ]]; then
    # hy2://password@server:port/?sni=...&insecure=1
    # SNI берём из config[domain] если задан, иначе config[server]
    hy2_sni="${config[domain]:-${config[server]}}"
    hy2_params="sni=${hy2_sni}"
    [[ ${config[security]} == 'selfsigned' ]] && hy2_params="${hy2_params}&insecure=1"
    client_config="hy2://$(echo -n "${username}${users[${username}]}" | sha256sum | cut -d ' ' -f 1 | head -c 16)@${config[server]}:${config[port]}/?${hy2_params}#${username}"
  else
    client_config="vless://${users[${username}]}@${config[server]}:${config[port]}"
    client_config="${client_config}?security=$([[ ${config[security]} == 'reality' ]] && echo reality || { [[ ${config[security]} == 'notls' ]] && echo none || echo tls; })"
    client_config="${client_config}&encryption=none&headerType=none&type=${config[transport]}"
    if [[ ${config[security]} != 'notls' ]]; then
      client_config="${client_config}&alpn=$([[ ${config[transport]} == 'ws' ]] && echo 'http/1.1' || echo 'h2,http/1.1')"
      client_config="${client_config}&fp=chrome&sni=${config[domain]%%:*}"
    ...
    [[ ${config[transport]} == 'ws' || ${config[transport]} == 'http' || ${config[transport]} == 'xhttp' ]] && client_config="${client_config}&path=%2F${config[service_path]}"
    [[ ${config[transport]} == 'xhttp' && -n ${config[host_header]} ]] && client_config="${client_config}&host=${config[host_header]}"
    [[ ${config[transport]} == 'ws'   ]] && client_config="${client_config}&host=${config[host_header]:-${config[server]}}"
    [[ ${config[transport]} == 'http' ]] && client_config="${client_config}&host=${config[host_header]:-${config[server]}}"
...
function upgrade {
  ...
  if [[ -r ${path[config]} ]]; then
    ...
    sed -i 's|core=singbox|core=xray|g' "${path[config]}"
...
function view_user_menu {
  ...
  while true; do
    ...
    whiptail --clear --backtitle "$BACKTITLE" --title "Детали ${username}" \
      --yes-button "QR-код" --no-button "Назад" \
      --yesno "${user_config}" $HEIGHT $WIDTH 3>&1 1>&2 2>&3
    if [[ $? -eq 0 ]]; then
      clear
      print_client_configuration "${username}"
      echo; echo "Нажмите Enter чтобы вернуться..."; read; clear
    fi
    if [[ $# -gt 0 ]]; then return 0; fi
  done
}
...
function configuration_menu {
  ...
  while true; do
    selection=$(whiptail --clear --backtitle "$BACKTITLE" --title "Настройки" \
      ...
      "4"  "Безопасность" \
      "5"  "SNI Домен" \
      "6"  "Транспорт VLESS (${config[transport]})" \
      "7"  "Путь VLESS" \
      "8"  "Host VLESS" \
      "9"  "WARP" \
    ...
    case $selection in
      ...
      5 ) config_sni_domain_menu ;;
      6 ) config_transport_menu ;;
      7 ) config_path_menu ;;
      8 ) config_host_menu ;;
      9 ) config_warp_menu ;;
...
function config_protocol_menu {
  ...
  while true; do
    ...
    if [[ ${proto} == 'hysteria2' && ${config[security]} == 'reality' ]]; then
      message_box 'Ошибка' 'hysteria2 несовместим с reality. Выберите letsencrypt или selfsigned.'
      continue
    fi
...
function config_transport_menu {
  local transport
  if [[ ${config[protocol]} == 'hysteria2' ]]; then
    message_box "Транспорт" "Hysteria2 всегда использует QUIC/UDP. Транспорт для VLESS не применяется."
    return
  fi
  while true; do
...
function config_sni_domain_menu {
  local sni_domain
  local hint=""
  [[ ${config[protocol]} == 'hysteria2' ]] && hint=" (используется в hy2:// ссылке)"
  while true; do
...
function config_security_menu {
  ...
  while true; do
    ...
    if [[ ! ${config[server]} =~ ${regex[domain]} && ${security} == 'letsencrypt' ]]; then
      message_box 'Ошибка' 'Для letsencrypt нужен валидный домен сервера.'
      continue
    fi
    if [[ ${config[transport]} == 'ws' && ${security} == 'reality' ]]; then
      message_box 'Ошибка' 'ws несовместим с reality.'
      continue
    fi
...
function config_path_menu {
  local user_path
  if [[ ${config[protocol]} == "hysteria2" ]]; then
    message_box "Путь" "Путь применяется только к VLESS."
    return
  fi
  while true; do
...
function config_host_menu {
  local host
  if [[ ${config[protocol]} == "hysteria2" ]]; then
    message_box "Host" "Host применяется только к VLESS."
    return
  fi
  while true; do
