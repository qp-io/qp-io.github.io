...
defaults[warp_interface_ipv6]=""defaults[security]=reality
...
[-r|--restart] [--enable-safenet=true|false] [--port=<порт>] [--enable-warp=true|false]
[--warp-license=<лицензия>] [--security=reality|letsencrypt|selfsigned|notls] [-m|--menu] [--show-server-config] [--add-user=<имя>] [--lists-users][--show-user=<имя>] [--delete-user=<имя>] [--backup] [--restore=<url|файл>] [--backup-password=<пароль>] [-u|--uninstall]
[--path=<путь>] [--host=<хост>]"
echo ""
...
-d|--domain)
args[domain]="$2"
[[ ${args[domain]} =~ ${regex[domain_port]} ]] || { echo "Неверный домен"; return 1; }shift 2 ;;
--server)
args[server]="$2"
...
shift 2 ;;
--list-users) args[list_users]=true; shift ;;
--show-user) args[show_config]="$2"; shift 2 ;;
--delete-user) args[delete_user]="$2"; shift 2 ;;--backup) args[backup]=true; shift ;;
--restore)
args[restore]="$2"
...
[[ ! -r "${path[config]}" ]] && { generate_keys; return 0; }
while IFS= read -r line; do
[[ "${line}" =~ ^\s*# || "${line}" =~ ^\s*$ ]] && continuekey=$(echo "$line" | cut -d "=" -f 1); value=$(echo "$line" | cut -d "=" -f 2-)
...
[[ ${config[protocol]} == 'hysteria2' && ${config[transport]} != '' ]] && { echo "Hysteria2 не поддерживает транспорты"; exit 1; }
...
which qrencode whiptail jq xxd zip unzip >/dev/null 2>&1 && return 0if which apt >/dev/null 2>&1; then
...
# VLESS: TCP + опционально 80 для reality/letsencrypt
echo "ports:" >> "${path[compose]}"
[[ ${config[security]} == 'reality' && ${config[port]} -eq 443 ]] && echo "- 80:8080" >> "${path[compose]}"echo "- ${config[port]}:8443" >> "${path[compose]}"
...
- ./certbot/data:/etc/letsencrypt- ./$(dirname "${path[server_pem]#${config_path}/}"):/certificate
...
defaultsoption http-server-close
timeout connect 5s
timeout client 50s
...
cat >"${path[certbot_startup]}" <<EOF#!/bin/sh
trap exit TERM
...
RUN apk add --no-cache docker-cli-compose curl bash newt libqrencode-tools sudo openssl jq zip unzipRUN pip install --no-cache-dir python-telegram-bot==22.3 qrcode[pil]==8.2
CMD [ "python", "./tgbot.py" ]
EOF
}
...
"outbounds":[
{"protocol":"freedom","tag":"internet"},$([[ ${config[warp]} == ON ]] && echo "${warp_object}" || true)
...
[[ ${config[security]} != 'notls' ]] && {
client_config="${client_config}&alpn=$([[ ${config[transport]} == 'ws' ]] && echo 'http/1.1' || echo 'h2,http/1.1')&fp=chrome&sni=${config[domain]%%:*}"
...
users[$username]=$(cat /proc/sys/kernel/random/uuid); update_users_file
whiptail --clear --backtitle "$BACKTITLE" --title "Добавить пользователя" --yes-button "Просмотр" --no-button "Назад" --yesno "Пользователь ${username} создан." $HEIGHT $WIDTH 3>&1 1>&2 2>&3
[[ $? -ne 0 ]] && break; view_user_menu "${username}"done
}
...
local title=$1 options selection
options=$(dict_expander users)selection=$(whiptail --clear --noitem --backtitle "$BACKTITLE" --title "$title" --menu "Пользователь" $HEIGHT $WIDTH $CHOICE_HEIGHT $options 3>&1 1>&2 2>&3)
...
[[ $? -ne 0 ]] && break
case $selection in
1) config_protocol_menu ;; 2) config_server_menu ;; 3) config_port_menu ;; 4) config_security_menu ;; 5) config_sni_domain_menu ;;
6) [[ ${config[protocol]} == 'vless' ]] && config_transport_menu ;;7) [[ ${config[protocol]} == 'vless' ]] && config_path_menu ;;
...
local sni_domain
while true; do
sni_domain=$(whiptail --clear --backtitle "$BACKTITLE" --title "SNI Домен" --inputbox "SNI:" $HEIGHT $WIDTH "${config[domain]}" 3>&1 1>&2 2>&3)
[[ $? -ne 0 ]] && break[[ ! $sni_domain =~ ${regex[domain_port]} ]] && { message_box "Ошибка" "Неверный домен"; continue; }
...
[[ $? -ne 0 ]] && break
[[ $tgbot == 'Выключить' ]] && { config[tgbot]=OFF; update_config_file; return; }
config[tgbot]=ONwhile true; do
...
[[ -z "$(${docker_cmd} ls | grep "${path[compose]}" | grep running || true)" ]] && { restart_docker_compose; return; }
${docker_cmd} --project-directory ${config_path} -p ${compose_project} ps --services "$1" | grep "$1" && ${docker_cmd} --project-directory ${config_path} -p ${compose_project} restart --timeout 2 "$1"
}
function warp_api {
...
[[ "${restart[tgbot]}" == 'true' && "${config[tgbot]}" == 'ON' ]] && restart_tgbot_compose
[[ "${config[tgbot]}" == 'OFF' ]] && ${docker_cmd} --project-directory ${config_path}/tgbot -p ${tgbot_project} down --remove-orphans --timeout 2 >/dev/null 2>&1 || true
[[ "${restart[compose]}" == 'true' ]] && { restart_docker_compose; return; }for key in "${!restart[@]}"; do [[ $key != 'none' && $key != 'tgbot' ]] && restart_container "${key}"; done
}
...
jq . "${docker_config}" &>/dev/null || { echo '{"experimental": true, "ip6tables": true}' | jq . > "${docker_config}"; config_modified=true; } || {
jq 'if .experimental != true or .ip6tables != true then .experimental = true | .ip6tables = true else . end' "${docker_config}" | jq . > "${temp_file}" && { cmp --silent "${docker_config}" "${temp_file}" || { mv "${temp_file}" "${docker_config}"; config_modified=true; }; }; }
firm -f "${temp_file}"
