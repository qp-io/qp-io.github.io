...
function show_help {
  echo ""
  echo "Использование: reality-ezpz.sh [-t|--transport=tcp|http|xhttp|grpc|ws|hysteria2] [-d|--domain=<домен>] [--server=<сервер>] [--regenerate] [--default]
  ...
  echo "  -c  --core <xray>          Выбрать ядро (только xray, по умолчанию: ${defaults[core]})"
    ...
    case $1 in
      -t|--transport)
        ...
        case ${args[transport]} in
          tcp|http|xhttp|grpc|ws|hysteria2)
            shift 2
            ;;
          *)
...
function build_config {
  ...
  if [[ ${config[transport]} == 'hysteria2' && ${config[security]} == 'reality' ]]; then
    echo 'Вы не можете использовать транспорт "hysteria2" с "reality". Используйте selfsigned.'
    exit 1
  fi
...
services:
  engine:
    ...
    $([[ ${config[security]} == 'reality' || ${config[security]} == 'notls' || ${config[transport]} == 'hysteria2' ]] && echo "ports:" || true)
    ...
    $([[ ${config[transport]} == 'hysteria2' ]] && echo "- ${config[port]}:8443/udp" || true)
    $([[ ${config[security]} != 'reality' && ${config[security]} != 'notls' && ${config[transport]} != 'hysteria2' ]] && echo "expose:" || true)
    $([[ ${config[security]} != 'reality' && ${config[security]} != 'notls' && ${config[transport]} != 'hysteria2' ]] && echo "- 8443" || true)
    restart: always
    environment:
      TZ: Etc/UTC
    volumes:
    - ./${path[engine]#${config_path}/}:/etc/${config[core]}/config.json
    $([[ ${config[security]} != 'reality' && ${config[security]} != 'notls' ]] && { [[ ${config[transport]} == 'http' ]] || [[ ${config[transport]} == 'tcp' ]] || [[ ${config[transport]} == 'hysteria2' ]]; } && echo "- ./${path[server_crt]#${config_path}/}:/etc/${config[core]}/server.crt" || true)
    $([[ ${config[security]} != 'reality' && ${config[security]} != 'notls' ]] && { [[ ${config[transport]} == 'http' ]] || [[ ${config[transport]} == 'tcp' ]] || [[ ${config[transport]} == 'hysteria2' ]]; } && echo "- ./${path[server_key]#${config_path}/}:/etc/${config[core]}/server.key" || true)
    networks:
    - reality
$(if [[ ${config[security]} != 'reality' && ${config[security]} != 'notls' && ${config[transport]} != 'hysteria2' ]]; then
echo "
  nginx:
    image: ${image[nginx]}
    expose:
...
function generate_engine_config {
  ...
  # ── Hysteria 2 inbound (нативный протокол Xray v26.3.27) ──────────────
...
EOF
    ...
    # ── VLESS inbound (tcp / http / grpc / ws / xhttp) ────────────────────
    for user in "${!users[@]}"; do
      if [ -n "$users_object" ]; then
        users_object="${users_object},"$'\n'
      fi
...
function generate_config {
  ...
  # Для hysteria2: сертификат нужен напрямую в engine (без haproxy)
  if [[ ${config[security]} != "reality" && ${config[security]} != "notls" && ${config[transport]} == 'hysteria2' ]]; then
    mkdir -p "${config_path}/certificate"
...
function show_server_config {
  local server_config
  server_config="Адрес сервера: ${config[server]}"
...
function configuration_menu {
  ...
  while true; do
    selection=$(whiptail --clear --backtitle "$BACKTITLE" --title "Настройки" \
      ...
      "7" "Узел (Host)" \
      "8" "WARP" \
      "9" "Безопасный интернет" \
      "10" "Telegram Бот" \
      "11" "Перезагрузка служб" \
      "12" "Перегенерация ключей" \
      "13" "Сброс настроек" \
      "14" "Создать бэкап" \
      "15" "Восстановить бэкап" \
      3>&1 1>&2 2>&3)
    if [[ $? -ne 0 ]]; then
      break
    fi
...
function config_transport_menu {
  ...
  while true; do
    ...
    if [[ ${transport} == 'hysteria2' && ${config[security]} == 'reality' ]]; then
      message_box 'Ошибка конфигурации' 'Вы не можете использовать транспорт "hysteria2" с сертификатом "reality". Используйте selfsigned.'
      continue
    fi
    if [[ ${transport} == 'hysteria2' && ${config[security]} == 'letsencrypt' ]]; then
      message_box 'Ошибка конфигурации' 'Для hysteria2 используйте "selfsigned". letsencrypt несовместим с UDP.'
      continue
    fi
...
function config_security_menu {
  ...
  while true; do
    ...
    if [[ ${config[transport]} == 'hysteria2' && ${security} == 'reality' ]]; then
      message_box 'Ошибка конфигурации' 'Вы не можете использовать сертификат "reality" с транспортом "hysteria2". Используйте selfsigned.'
      continue
    fi
    if [[ ${config[transport]} == 'hysteria2' && ${security} == 'letsencrypt' ]]; then
      message_box 'Ошибка конфигурации' 'Для hysteria2 используйте "selfsigned". letsencrypt несовместим с UDP.'
      continue
    fi
