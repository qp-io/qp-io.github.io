...
# Матрица совместимости транспорт → ядра и security
TRANSPORT_COMPAT = {
...
async def apply_setting(update: Update, context: ContextTypes.DEFAULT_TYPE, param: str, val: str):
    ...
    if param == "transport":
        ...
        if core not in t_info.get("cores", []):
            allowed_cores = "/".join(t_info.get("cores", ["xray"]))
            await context.bot.send_message(chat_id, f"⚠️ Транспорт <b>{val}</b> работает только с ядром <b>{allowed_cores}</b>. Смените ядро.", parse_mode="HTML")
    ...
    if param == "security":
        ...
        allowed_sec = t_info.get("security", []) if core in t_info.get("cores", []) else []
        if val not in allowed_sec:
            await context.bot.send_message(chat_id, f"⚠️ Security <b>{val}</b> несовместим с транспортом <b>{transport}</b>.", parse_mode="HTML")
            await send_settings_menu(context.bot, chat_id)
            return
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "main":
        ...
        await context.bot.send_message(chat_id, "⏳ Включаю WARP...\nМожет занять 1–2 минуты.")
        rc, out = run_script("--warp ON", timeout=240)
        snippet = out if len(out) < 3900 else out[:3900] + "\n...(truncated)"
        await context.bot.send_message(
        ...
        if arg == "core":
            ...
            c2 = read_config()
            core = c2.get("core", "xray")
            # Показываем только транспорты совместимые с текущим ядром
            ...
            c2 = read_config()
            transport = c2.get("transport", "tcp")
            core = c2.get("core", "xray")
            # Показываем только security совместимые с текущим транспортом и ядром
            t_info = TRANSPORT_COMPAT.get(transport, {})
            if core in t_info.get("cores", []):
                sec_opts = t_info.get("security", [])
            else:
                sec_opts = ["notls"]  # fallback
            kb = [
